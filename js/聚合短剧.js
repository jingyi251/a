...
const aggConfig = {
  ...
  platformList: [
    { name: '甜圈短剧', id: '甜圈' }, { name: '七猫短剧', id: '七猫' }, { name: '番茄短剧', id: '番茄' },
    { name: '星芽短剧', id: '星芽' }, { name: '锦鲤短剧', id: '锦鲤' }, { name: '西饭短剧', id: '西饭' }, 
    { name: '百度短剧', id: '百度' }, { name: '围观短剧', id: '围观' }, { name: '碎片剧场', id: '碎片' }
  ],
...
const ruleFilterDef = {
  ...
  星芽: { area: '1' }, 西饭: { area: '' }, 七猫: { area: '0' }, 围观: { area: '' }, 碎片: { area: '' }
};
  ...
  try {
    ...
    switch (platId) {
      ...
      case '七猫': {
        ...
        const headers = { ...await getHeaderX(), ...aggConfig.headers.default };
        const res = safeParse(await req(url, { method: 'GET', headers }));
        if (res && res.data) {
          const data = res.data;
          vod = {
            vod_id: id,
    ...
    try {
      ...
      switch (p.id) {
        ...
        case '七猫': {
          ...
          const headers = { ...await getHeaderX(), ...aggConfig.headers.default };
          res = await req(url, { method: 'GET', headers, timeout });
          data = (safeParse(res)?.data?.list || []).map(item => ({
            vod_id: `七猫@${encodeURIComponent(item.playlet_id)}`,
